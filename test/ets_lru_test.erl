...
lifecyle_test_() ->
    ...
    fun({ok, LRU}) ->
        [
            {"ets_lru:start_link/2 returned an LRU", ?_assert(is_pid(LRU))},
            {"Destroyed the LRU ok", ?_assertEqual(ok, ets_lru:stop(LRU))}
        ]
    end}.
    ...
    [
        ...
        fun({ok, _LRU}) ->
            [{"Dirty lookup returned the inserted value",
                ?_assertEqual({ok, bar}, ets_lru:lookup_d(test_lru, foo))}]
        end,
        fun({ok, LRU}) ->
            [{"Lookup returned the inserted value",
                ?_assertEqual({ok, bar}, ets_lru:lookup(LRU, foo))},
...
lru_match_test_() ->
    ...
    [
        fun({ok, LRU}) ->
            [{"Empty match", ?_assertEqual([], ets_lru:match(LRU, a, '$1'))}]
        end,
        fun({ok, LRU}) ->
            ets_lru:insert(LRU, b, {x, y}),
            [{"Single match",
                ?_assertEqual([[x, y]], ets_lru:match(LRU, b, {'$1', '$2'}))}]
        end,
        fun({ok, LRU}) ->
        ...
        fun({ok, LRU}) ->
            ...
            [{"Single match_object (registered)",
                ?_assertEqual([42], ets_lru:match_object(test_lru,ans,'$1'))},
            {"Single match_object (pid)",
                ?_assertEqual([42], ets_lru:match_object(LRU, ans, '$1'))}]
        end,
        fun({ok, LRU}) ->
...
test_good_opts(Opts, Cfg) ->
    Msg = io_lib:format("LRU created ok with options: ~w", [Opts]),
    case Cfg of
    {ok, LRU} -> [{lists:flatten(Msg), ?_assert(is_pid(LRU))}];
    ErrorMsg -> [{lists:flatten(Msg), ?_assertEqual(ok, ErrorMsg)}]
    end.
...
test_limits([{max_objects, N}], {ok, LRU}) ->
    [{"Max object count ok", ?_assert(insert_kvs(size, LRU, 100 * N, N))}];
test_limits([{max_size, N}], {ok, LRU}) ->
    [{"Max size ok", ?_assert(insert_kvs(memory, LRU, 10 * N, N))}];
test_limits([{max_lifetime, N}], {ok, LRU}) ->
    [
        {"Expire leaves new entries", {?LINE, fun() ->
            ets_lru:insert(LRU, foo, bar),
            ?_assertEqual({ok, bar}, ets_lru:lookup(LRU, foo))
        end}},
        {"Entry was expired", {?LINE, fun() ->
